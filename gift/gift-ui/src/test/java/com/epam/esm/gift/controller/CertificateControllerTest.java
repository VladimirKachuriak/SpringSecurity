...
class CertificateControllerTest {
    ...
    void getByTags() throws Exception {
        ...
        when(certificateService.getAll(1, List.of("Tag1", "Tag2"))).thenReturn(List.of(certificate));
        mockMvc.perform(get("/certificate").param("page", "1")
                        .param("tags", "Tag1")
                        .param("tags", "Tag2"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$[0].name", is("METRO")))
                .andExpect(jsonPath("$[0].id", is(3)));
        verify(certificateService, times(1)).getAll(1, List.of("Tag1", "Tag2"));
    }
    ...
    void updateDuration() throws Exception {
        ...
        when(certificateService.update(certificate)).thenReturn(true);
        when(certificateService.getById(3)).thenReturn(certificate);
        mockMvc.perform(put("/certificate/3/updateDuration").
                        param("duration","30"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_PLAIN))
                .andExpect(jsonPath("$", is("Resource updated successfully")));
    ...
    void updatePrice() throws Exception {
        ...
        when(certificateService.update(certificate)).thenReturn(true);
        when(certificateService.getById(3)).thenReturn(certificate);
        mockMvc.perform(put("/certificate/3/updatePrice").
                        param("price","55"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_PLAIN))
                .andExpect(jsonPath("$", is("Resource updated successfully")));
